...
## Task Execution
- Work as a group in teams of 4.
- Airtable link
- Write the CI/CD workflow for deployment.
- Request/Response/Model naming convention should be snake_case.
- To request a server for deployment use the command `/request-server`
...
- **Circuit Breaker**: Prevents total system failure when a service (like SMTP or FCM) goes down.
- **Retry System**: Retry failed messages with exponential backoff and move permanently failed messages to the dead-letter queue.
- **Service Discovery**: Allows services to find each other dynamically.
- **Health Checks**: Each service should have a `/health` endpoint for status monitoring.
- **Idempotency**: Prevent duplicate notifications by using unique request IDs.
...
Use Draw.io, Miro, or Lucidchart (free tools).
...
- **Deadline**: Wednesday 12th of November, 2025. 11:59pm GMT +1 (WAT)
