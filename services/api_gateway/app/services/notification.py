...
class NotificationService:
    ...
    async def submit_notification(
        ...
        notification_requests_total.labels(notification_type=notification_type).inc()

        # Check idempotency
        if request.request_id:
            idempotency_checks_total.labels(result="checked").inc()
            ...
            if cached_response:
                ...
                duplicate_requests_total.labels(notification_type=notification_type).inc()
                ...
                idempotency_checks_total.labels(result="unique").inc()
        else:
            idempotency_checks_total.labels(result="skipped").inc()
        ...
        try:
            ...
            notification_errors_total.labels(
                notification_type=notification_type, error_type="circuit_breaker"
            ).inc()
            ...
            # Re-raise HTTP exceptions (metrics already tracked)
            raise

        except Exception as e:
            # Track error
            notification_errors_total.labels(
                notification_type=notification_type, error_type="processing_error"
            ).inc()
